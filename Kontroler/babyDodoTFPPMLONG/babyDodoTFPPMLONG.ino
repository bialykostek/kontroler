...
const int NNinpLen = 19;

float NNinput[NNinpLen*(reccurentLength+1)];
float NNoutput[5];
float NNlearnOut[5] = {
  0,
  0,
...
void loop() {
  ...
  if (!emergency && millis() - NNdataTimer > 50) {
    ...
    for(int i=0; i < reccurentLength*NNinpLen; i++){
      NNinput[NNinpLen*(reccurentLength+1)- 1 - i] = NNinput[NNinpLen*(reccurentLength+1)- 1 - i-NNinpLen];
    }
    
    
    NNinput[14] = NNlearnOut[0]; //pwm
    NNinput[15] = NNlearnOut[1];
    NNinput[16] = NNlearnOut[2];
    NNinput[17] = NNlearnOut[3];
    NNinput[18] = NNlearnOut[4];

    NNinput[0] = millis();//(roll + 180) / 360 + millis()/10;
    NNinput[1] = (pitch + 180) / 360;

    NNinput[2] = (float) gspeed / 25000;
    NNinput[3] = ((float) GPSaltitude / 1000 - altiPressOffset) / 40;
    NNinput[4] = vPitot / 20;
    ...
    NNinput[7] = (angles[2] + pi) / 2 / pi;

    NNinput[13] = distanceToCurrent / 500;
    ...
    for(int i=0; i < NNinpLen*(reccurentLength+1); i++){
